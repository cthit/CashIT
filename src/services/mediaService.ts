...
export default class MediaService {
  ...
  static async saveNamed(file: Blob, name: string, types: MediaType[]) {
    const meta = await this.save(file, types);
    if (!meta) return null;

    // Return file info if it already exists in database
    const existsDb = await prisma.namedMedia.findUnique({
      where: {
        sha256_name: {
          sha256: meta.sha256,
          name
    ...
    return await prisma.namedMedia.create({
      data: {
        name,
        media: {
          connect: {
  ...
  static async loadNamed(id: number) {
    const media = await prisma.namedMedia.findUnique({
      where: {
        id
      },
      include: {
    ...
    const info = await this.load(media.media.sha256);
